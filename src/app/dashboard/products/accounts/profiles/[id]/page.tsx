...
export default function Page({ params }: { params: { id: string } }) {
  const link = "/dashboard/products/accounts/profile/";
  const licensePath = "streaming/profile";
  const providerPath = "streaming/account";
  const id = params.id;

  const [item, setItem] = useState<StreamingProfile>();
  const [provider, setProvider] = useState<StreamingAccount>(null);
  ...
  async function getItems() {
    ...
    if (response.ok) {
      const json: StreamingProfile = await response.json();
      setItem(json);
      if (json.idStreamingAccount) {
        const response2 = await fetchProductById(json.idStreamingAccount, providerPath);

        if (response2.ok) {
          const json2: StreamingAccount = await response2.json();
          setProvider(json2);
        }
      }
    } else {
        ...
        <div className="order-2 md:order-1">
          <Breadcrumbs
            breadcrumbs={[
              ...
              {
                label: "Ver",
                href: `/dashboard/products/accounts/profiles/${id}`,
                active: true,
              },
            ]}
          />
              ...
              <div className="w-full max-w-md bg-white dark:bg-gray-800 order-2 md:order-1">
                <div className="flex items-center justify-between mb-4">
                  <h5 className="text-xl font-bold leading-none text-gray-900 dark:text-white">
                    {provider ? `${provider.id} - ${provider.streamingProvider.name}` : "Sin definir proveedor"}
                  </h5>
                  :
                </div>
