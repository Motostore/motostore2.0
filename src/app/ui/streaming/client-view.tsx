...
export default function ClientView({ items, loading }: any) {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      {/* ESTADO VACÍO PARA CLIENTE (SIN BOTONES DE REGISTRAR) */}
      {!loading && items.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center opacity-60">
            ...
            <p className="text-sm text-slate-400">Tus compras aparecerán aquí.</p>
            {/* Aquí NO hay botón de "Registrar Cuenta". Solo el admin puede hacer eso. */}
        </div>
      )}
          ...
          <div key={item.id} className="group bg-white rounded-2xl p-6 shadow-sm border border-slate-100 hover:shadow-xl hover:shadow-slate-200/50 hover:border-red-100 transition-all duration-300 relative overflow-hidden">
            ...
            <div className="mt-5 pt-4 border-t border-slate-100 flex items-center justify-between text-xs">
                <div className="flex items-center gap-1 text-slate-500 font-medium">
                    <ClockIcon className="w-4 h-4 text-slate-300"/> Vence: <span className="text-slate-900 font-bold">{formatDateToLocal(item.dueDate)}</span>
                </div>
            </div>
          </div>
        ))}
