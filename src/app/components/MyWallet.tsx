...
export default function Wallet() {
  ...
  const effectiveUserId: string | number | null = (envUserId ? envUserId : pickBestUserId(user));
  ...
  const urlsToTry = useMemo(() => {
    const uid = encodeURIComponent(String(effectiveUserId ?? ''));
    if (customUrl) {
  ...
  const parseBalancePayload = (raw: string) => {
    ...
    if (/^-?\d+(\.\d+)?$/.test(raw)) {
      ...
      try {
        ...
        value =
          j?.balance ?? j?.saldo ?? j?.amount ?? j?.monto ??
          (j?.data && (j.data.balance ?? j.data.saldo ?? j.data.amount ?? j.data.monto)) ??
          (typeof j === 'number' ? j : null);

        profitValue =
          j?.profit ?? j?.profits ?? j?.utilities ?? j?.utilidad ?? j?.utilidades ??
          j?.earnings ?? j?.ganancias ?? j?.profitUsd ?? j?.utilitiesUsd ??
          (j?.data && (
            j.data.profit ?? j.data.profits ?? j.data.utilities ?? j.data.utilidad ??
            j.data.utilidades ?? j.data.earnings ?? j.data.ganancias
          )) ?? null;
      } catch {}
    }
  ...
  async function fetchWithOptionalToken(url: string) {
    for (const tryToken of [true, false]) {
      ...
      try {
        ...
        if (!r.ok) { setHttpCode(r.status); continue; }
        setHttpCode(null);
        const raw = (await r.text()).trim();
        const { value, profitValue } = parseBalancePayload(raw);
        if (typeof value === 'number' && !Number.isNaN(value)) {
          setAmount(value);
          setProfit(typeof profitValue === 'number' && !Number.isNaN(profitValue) ? profitValue : null);
          setStatus('ok');
          return true;
        }
      } catch {}
  ...
  async function fetchBalance() {
    if (!effectiveUserId) {
      setStatus('error'); setHttpCode(0); return;
    }
    setStatus('loading'); setHttpCode(null);
  ...
  useEffect(() => { fetchBalance(); /* eslint-disable-next-line */ }, [effectiveUserId, customUrl, sendCookies, token]);
  ...
  const saldoLabel =
    ...
    : status === 'ok' ? formatUSD(amount)
    : httpCode ? `Error ${httpCode}` : 'Error';

  const utilidadesLabel =
    status === 'loading' ? '—'
    : status === 'ok' ? formatUSD(profit ?? 0)
    : '—';

  return (
    <div ref={wrapRef} className="relative inline-block">
      <button onClick={() => setMenuOpen(o => !o)} className="inline-flex flex-col items-start gap-0.5 p-0 text-left">
        <div className="flex items-center gap-1 text-sm">
          <CurrencyDollarIcon className="relative -top-[1px] h-3.5 w-3.5 text-emerald-600/80" aria-hidden="true" />
        ...
        <div className="flex items-center gap-1 text-[11px] text-gray-500 leading-none">
          <ArrowTrendingUpIcon className="relative -top-[1px] h-3 w-3" aria-hidden="true" />
          <span>Utilidades:</span>
          <span className="text-gray-800">{utilidadesLabel}</span>
        </div>
      </button>

      <div className={['absolute right-0 z-50 mt-2 w-44 rounded-lg border border-gray-200 bg-white/95 backdrop-blur p-1.5 shadow-sm', menuOpen ? 'block' : 'hidden'].join(' ')}>
        <button onClick={topUpMin} disabled={topupLoading || status === 'loading'} className="w-full rounded-md px-2.5 py-1.5 text-left text-[13px] hover:bg-gray-100 disabled:opacity-50">
          {topupLoading ? 'Procesando…' : `Recargar mín. (${MIN_TOPUP})`}
        </button>
        <button onClick={fetchBalance} disabled={status === 'loading'} className="mt-0.5 w-full rounded-md px-2.5 py-1.5 text-left text-[13px] hover:bg-gray-100 disabled:opacity-50">
          Actualizar saldo
        </button>
      </div>
    </div>
